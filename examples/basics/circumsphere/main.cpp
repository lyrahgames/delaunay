...
void generate_tetrahedron_and_check_points() {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    tetra_vertices[i] =
        vertex{tetrahedron[i].x, tetrahedron[i].y, tetrahedron[i].z, 0, 0, 0};
  }

  // Get circumsphere.
  const auto s = delaunay::experimental_3d::circumsphere(
      tetrahedron[0], tetrahedron[1], tetrahedron[2], tetrahedron[3]);

  for (auto& v : vertices) {
    const point p{2 * dist(rng), 2 * dist(rng), 2 * dist(rng)};
    if (delaunay::experimental_3d::intersection(s, p))
      v = vertex{p.x, p.y, p.z, 1, 0, 0};
    else
      v = vertex{p.x, p.y, p.z, 0, 0, 1};
  }
}
...
int main(void) {
  ...
  auto window =
      glfwCreateWindow(width, height, "Circumsphere Viewer", nullptr, nullptr);
